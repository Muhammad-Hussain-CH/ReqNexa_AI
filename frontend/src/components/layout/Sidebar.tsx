    ...
    <aside className={`${open ? "translate-x-0" : "-translate-x-full md:translate-x-0"} md:w-64 w-64 shrink-0 border-r dark:border-gray-700 h[calc(100vh-56px)] p-4 text-sm bg-white dark:bg-gray-800 fixed md:static top-14 left-0 transition-transform duration-300`}> 
      ...
      <nav className="space-y-1">
        {nav.map((n) => {
          ...
          return (
            <Link key={n.to} to={n.to} className={`flex items-center gap-2 px-3 py-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition ${active ? "bg-gray-100 dark:bg-gray-700 text-primary" : "text-gray-700 dark:text-gray-200"}`} onClick={onClose}>
              <Icon className="w-4 h-4" /> {n.label}
            </Link>
          );
        })}
      </nav>
      <div className="absolute bottom-4 left-4 right-4 flex items-center gap-3 p-3 border dark:border-gray-700 rounded">
