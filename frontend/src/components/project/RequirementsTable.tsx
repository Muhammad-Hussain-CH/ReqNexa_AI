  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        <select className="px-3 py-2 border rounded" value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)}>
          <option value="">All Types</option>
          <option value="functional">Functional</option>
          <option value="non_functional">Non-Functional</option>
        </select>
        <select className="px-3 py-2 border rounded" value={priorityFilter} onChange={(e) => setPriorityFilter(e.target.value)}>
        ...
        <select className="px-3 py-2 border rounded ml-auto" value={pageSize} onChange={(e) => setPageSize(Number(e.target.value) as PageSize)}>
      ...
      {error && !isLoading && (
        <div className="rounded border bg-white p-4">
          ...
          {onRetry && <button className="mt-2 px-3 py-1.5 rounded border" onClick={onRetry}>Retry</button>}
        </div>
      )}

      {!isLoading && !error && paged.length === 0 && (
        <div className="h-48 grid place-items-center text-gray-500">
            ...
            <tbody>
              {paged.map((r) => (
                <tr key={r.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-2 text-xs text-gray-600">{r.category || "-"}</td>
                  ...
                  <td className="p-2">
                    <div className="flex items-center gap-2">
                      <button className="p-1 rounded hover:bg-gray-100" onClick={() => onEdit && onEdit(r)}><Edit className="w-4 h-4" /></button>
                      <button className="p-1 rounded hover:bg-gray-100" onClick={() => onDelete && onDelete(r)}><Trash className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {selected.length > 0 && (
        <div className="flex items-center gap-2">
          ...
          <button className="px-3 py-1.5 rounded border" onClick={() => onBulkUpdate && onBulkUpdate(selected)}>Bulk Update</button>
        </div>
      )}

      <div className="flex items-center justify-end gap-2">
        <button className="px-3 py-1.5 rounded border" onClick={() => setPage((p) => Math.max(1, p - 1))}>Prev</button>
        <div className="text-sm">Page {page} / {totalPages}</div>
        <button className="px-3 py-1.5 rounded border" onClick={() => setPage((p) => Math.min(totalPages, p + 1))}>Next</button>
      </div>
    </div>
  );
}
