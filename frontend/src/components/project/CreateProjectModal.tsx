...
export default function CreateProjectModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 grid place-items-center z-50">
      <form className="w-full max-w-md bg-white rounded shadow p-6 space-y-3" onSubmit={onSubmit}>
        ...
        <div>
          ...
          <select className="w-full px-3 py-2 border rounded" value={type} onChange={(e) => setType(e.target.value)}>
        ...
        <div>
          ...
          <textarea className="w-full px-3 py-2 border rounded resize-none" rows={4} value={description} onChange={(e) => setDescription(e.target.value)} />
        </div>
        <div className="flex items-center justify-end gap-2">
          <button type="button" className="px-3 py-2 rounded border hover:bg-gray-50" onClick={onClose}>Cancel</button>
