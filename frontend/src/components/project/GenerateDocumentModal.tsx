      ...
      <form onSubmit={onSubmit} className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            ...
            <select className="w-full px-3 py-2 border rounded" value={type} onChange={(e) => { const t = e.target.value as any; setType(t); setFormat(t === "srs" ? "pdf" : t === "requirements_export" ? "csv" : "json"); }}>
          ...
          <div>
            ...
            <select className="w-full px-3 py-2 border rounded" value={format} onChange={(e) => setFormat(e.target.value as any)}>
          ...
          {type === "srs" && (
            <div>
              ...
              <select className="w-full px-3 py-2 border rounded" value={template} onChange={(e) => setTemplate(e.target.value as any)}>
        ...
        {type === "srs" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={include_toc} onChange={(e) => setIncludeToc(e.target.checked)} /> Include TOC</label>
            <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={include_glossary} onChange={(e) => setIncludeGlossary(e.target.checked)} /> Include Glossary</label>
            <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={include_traceability} onChange={(e) => setIncludeTraceability(e.target.checked)} /> Include Traceability</label>
          </div>
        )}
